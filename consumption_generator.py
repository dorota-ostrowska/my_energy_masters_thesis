...
class ConsumptionGenerator:
    ...
    def _open_connection_with_db(
        ...
        Optional[psycopg2.extensions.cursor], Optional[psycopg2.extensions.cursor]
    ]:
        """
    ...
    def add_reading_to_table(
        self, id_reading: int, time: str, used_energy: float, meter_id_meter: int
    ) -> None:
        """
        ...
        for meter in all_meters:
            ...
            self.add_reading_to_table(
                ...
                max(
                    0,
                    (
                        self.generate_energy_consumption(reading_date.hour)
                        + self.generate_noise()
                    ),
                ),
                meter[0],
